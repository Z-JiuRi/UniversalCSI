...
#
# 常用覆盖：
#   mapper=delta_mlp align_mode=affine epochs=400 gpus=0,4,6,7 bash mapper/scripts/run_mapper_combined_losses.sh
#   residual_mapping=0 mapper=hybrid_flow_mlp epochs=400 gpus=0,4,6,7 bash mapper/scripts/run_mapper_combined_losses.sh
...
launch_job() {
  ...
  exp_dir="mapper/exps_combined_losses/${mapper}/${align_tag}/${config_tag}/${source_name}_to_seed42_transnet_lr${lr}_ep${epochs}"
