...
class ConditionExtractor(nn.Module):
    def __init__(self, method="svd", code_dim=512, num_tokens=512,
    ...
    def _random(self, codes):
        ...
        if n >= self.num_tokens:
            ...
            out = codes[idx]
            mask = torch.ones(
                self.num_tokens, dtype=torch.bool, device=codes.device)
            return out, mask
    ...
    def forward(self, codes):
        ...
        for attn in self.layers:
            attn_update, _ = attn(q, kv, kv, need_weights=False)
            q = self.norm(q + attn_update)
...
class ParamFM(nn.Module):
    ...
    def forward(self, theta_tokens, t, meta, cond_tokens, cond_mask,
        ...
        for block in self.blocks:
            h = block(h)
        velocity = self.out(self.norm(h))
        if self.condition_inject != "hyper_lora":
            return velocity
        a = self.hyper_a(global_cond).view(
            self.hidden_dim, self.hyper_lora_rank)
        b = self.hyper_b(global_cond).view(
            self.hyper_lora_rank, self.token_size)
        alpha = torch.tanh(self.hyper_alpha(global_cond)).view(1)
        lowrank = self.norm(h).matmul(a).matmul(b)
        return velocity + alpha * lowrank


class DecoderGeneralizationFM(nn.Module):
    ...
    def encode_condition(self, condition_codes):
        cond_tokens, cond_mask = self.condition_extractor(condition_codes)
        enc_tokens, global_cond = self.condition_encoder(cond_tokens, cond_mask)
        return enc_tokens, cond_mask, global_cond

    def forward(self, theta_tokens, t, meta, condition_codes):
        cond_tokens, cond_mask, global_cond = self.encode_condition(condition_codes)
